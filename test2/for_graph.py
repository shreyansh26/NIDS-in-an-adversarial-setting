...
probas_ = dt.predict_proba(X_test_scaled)
np.set_printoptions(threshold=np.inf)
print(probas_)
print()
print(pd.Series(probas_[:,0]).unique())
print()
print(y_test_l)
print()
# Calculate FPR for normal class only
fpr_dt, tpr_dt, _ = roc_curve(y_test_l, probas_[:, 0], pos_label=1, drop_intermediate=False)
print(fpr_dt)
print()
print(tpr_dt)
...
probas2_ = dt.predict_proba(X_adv)
fpr_dt_adv, tpr_dt_adv, _ = roc_curve(y_test_l, probas2_[:, 0], pos_label=1, drop_intermediate=False)
